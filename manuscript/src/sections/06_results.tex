...
\begin{table}[t]
  ...
  \input{../assets/tables/main_benchmark_top5.tex}
\end{table}
...
Bootstrap confidence intervals are provided in Supplementary Table S1 and do
not alter the qualitative regime-level interpretation. A supplementary F1--MAE
diagnostic plot is provided in Supplementary Fig. S1.
...
\begin{table}[t]
  ...
  \input{../assets/tables/paired_wilcoxon_key_comparisons.tex}
\end{table}
...
under wider tolerances. These paired tests are preselected key comparisons and
are used as supporting evidence rather than as a family-wise multiple-testing
claim; a Holm-adjusted audit table is included in the reproducibility package.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.82\linewidth]{../assets/figures/fig05_detection_overlay.pdf}
  \caption{Representative detection windows: vertical lines mark true change
  points and markers show selected predictions.}
...
\begin{table}[t]
  ...
  \input{../assets/tables/same_noise_best_f1_summary.tex}
\end{table}

The same-noise experiment in Table~\ref{tab:same-noise-diagnostic} gives the
colored-noise boundary. This diagnostic is not a replacement for the frozen
universal-training benchmark: it uses separately trained noise-family-specific
Transformer runs and asks whether this restriction reverses the colored-noise
comparison. Training and testing Transformer separately within each \(D=1.0\)
noise family still leaves it below the best classical baseline in all five
cases. Noise-specific Transformer training alone therefore does not support a
universal colored-noise neural-advantage claim under the evaluated pipeline.

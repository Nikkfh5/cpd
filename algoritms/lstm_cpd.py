...
Используется бинарная классификация на скользящих окнах:
  ...
  - Обучение на синтетических данных дискретной стохастической карты
...
def load_notebook_data(
    ...
    """
    Загружает тестовые данные через генератор дискретной стохастической карты.

    Траектория строится обновлением
    x[t+1] = x[t] + sin(x[t])*dt + sqrt(D)*noise с нормированным шумом,
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    Группы аргументов:
        ...
        Параметры генератора: --dt, --D, --noise-type, --length, --seed

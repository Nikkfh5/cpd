"""
Адаптер LSTM-пайплайна для обнаружения точек разладки.

Оборачивает algoritms/lstm_cpd.py в унифицированный интерфейс CPD-пайплайна.
...
def run_lstm(
    ...
    **kwargs,
    ...
    """
    Обучает LSTM на синтетических данных и делает инференс на series.
    ...
    :param window_size: чётное целое >= 2 — размер скользящего окна LSTM
    ...
    :param num_layers: число слоёв LSTM
    ...
    try:
        ...
        raise ImportError("torch обязателен для LSTM-адаптера") from exc
    ...
    from algoritms.lstm_cpd import (
    ...
    model = LSTMChangePointDetector(input_size=1, hidden_size=hidden_size, num_layers=num_layers, dropout=dropout)

    if weights_path and Path(weights_path).exists():
        checkpoint = torch.load(weights_path, map_location=device)
        model.load_state_dict(checkpoint["state_dict"])
        model.to(device)
        print(f"LSTM: loaded weights from {weights_path}", flush=True)
    else:
        print(f"LSTM: device={device}, generating {n_train_seq} train seqs x {seq_length}...", flush=True)
        ...
        print(f"LSTM: train data ready ({len(x_train)} pts), generating {n_val_seq} val seqs...", flush=True)
        ...
        print(f"LSTM: val data ready ({len(x_val)} pts), building datasets...", flush=True)

        train_dataset = SlidingWindowDataset(x_train, cp_train, window_size)
        val_dataset = SlidingWindowDataset(x_val, cp_val, window_size)
        print(f"LSTM: {len(train_dataset)} train windows, {len(val_dataset)} val windows. Starting training.", flush=True)
        ...
        model_filename = (
            f"lstm_h{hidden_size}_l{num_layers}_seq{seq_length}"
        ...
        print(f"LSTM: model saved to {model_path}", flush=True)

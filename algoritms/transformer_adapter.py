"""
Адаптер Transformer-пайплайна для обнаружения точек разладки.

Оборачивает algoritms/transformer_cpd.py в унифицированный интерфейс CPD-пайплайна.
...
def run_transformer(
    ...
    **kwargs,
    ...
    """
    Обучает Transformer на синтетических данных и делает инференс на series.
    ...
    :param window_size: чётное целое >= 2 — размер скользящего окна
    ...
    :param d_model: размерность пространства представлений Transformer
    :param nhead: число голов multi-head attention
    :param num_layers: число слоёв TransformerEncoderLayer
    :param dim_feedforward: размерность FFN внутри энкодера
    :param dropout: вероятность dropout
    :param weight_decay: L2-регуляризация в оптимизаторе Adam
    :param t4_fraction: доля T4-последовательностей в обучающих данных
    ...
    try:
        ...
        raise ImportError("torch обязателен для Transformer-адаптера") from exc
    ...
    from algoritms.transformer_cpd import (
        TransformerChangePointDetector,
    ...
    print(
        f"Transformer: device={device}, d_model={d_model}, nhead={nhead}, "
        f"num_layers={num_layers}, dim_ff={dim_feedforward}",
        flush=True,
    )

    model = TransformerChangePointDetector(
        input_size=1,
        d_model=d_model,
        nhead=nhead,
    ...
    print(f"Transformer: {total_params:,} параметров", flush=True)

    if weights_path and Path(weights_path).exists():
        checkpoint = torch.load(weights_path, map_location=device)
        model.load_state_dict(checkpoint["state_dict"])
        model.to(device)
        print(f"Transformer: loaded weights from {weights_path}", flush=True)
    else:
        print(
            f"Transformer: generating {n_train_seq} train seqs x {seq_length} "
            f"(t4_fraction={t4_fraction})...",
            flush=True,
        )
        ...
        print(
            f"Transformer: train data ready ({len(x_train)} pts), "
            f"generating {n_val_seq} val seqs...",
            flush=True,
        )
        ...
        print(f"Transformer: val data ready ({len(x_val)} pts), building datasets...", flush=True)

        train_dataset = SlidingWindowDataset(x_train, cp_train, window_size)
        val_dataset = SlidingWindowDataset(x_val, cp_val, window_size)
        print(
            f"Transformer: {len(train_dataset)} train windows, "
            f"{len(val_dataset)} val windows. Starting training.",
            flush=True,
        )
        ...
        model_filename = (
            f"transformer_dm{d_model}_h{nhead}_l{num_layers}_seq{seq_length}"
        ...
        print(f"Transformer: model saved to {model_path}", flush=True)
